...
프로세스 탭은 제한적으로 MCP/ngrok start/stop/restart 버튼을 제공합니다. 전체 session stop은 별도 확인 페이지를 거쳐 UI에서도 요청할 수 있으며, 전체 session restart는 `scripts/dev_session.sh restart-session`으로 수행할 수 있습니다. MCP/ngrok 개별 제어는 UI 버튼 또는 `scripts/dev_session.sh start-service`, `stop-service`, `restart`로 수행할 수 있습니다.
