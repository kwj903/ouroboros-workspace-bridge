...
Phase 3-R1 treats the Phase 3 task-workspace orchestration flow as a first release checkpoint. The operator guide now documents the primary happy path: create a task worktree, run worker changes in `task-workspace` mode, call `workspace_prepare_safe_task_merge_and_wait`, approve the generated `/pending` merge proposal, propose and approve source-level validation, inspect `workspace_task_validation_result_hint`, record the reviewed validation status manually, archive runtime records, then preview and propose cleanup only when the conservative cleanup preview reports a ready candidate. This release checkpoint does not add public tools or change source apply, validation, cleanup, direct mode, or runner behavior; it stabilizes the operational sequence and leaves richer interactive UI controls and optional automatic validation recording as future enhancements.
